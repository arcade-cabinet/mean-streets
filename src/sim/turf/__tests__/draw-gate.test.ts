...
function makeState(
  ...
  const toughsInPlay = turfs.reduce(
    (sum, t) => sum + t.stack.filter((sc) => sc.card.kind === 'tough').length,
    0,
  );
  return {
...
describe('isModifierCard', () => {
  it('weapon is a modifier', () =>
    expect(isModifierCard(weapon('w1'))).toBe(true));
  it('drug is a modifier', () => expect(isModifierCard(drug('d1'))).toBe(true));
  it('currency is a modifier', () =>
    expect(isModifierCard(currency('c1'))).toBe(true));
  it('tough is NOT a modifier', () =>
    expect(isModifierCard(tough('t1'))).toBe(false));
});
...
describe('placement rules in enumerateLegalActions', () => {
  ...
  it('includes modifier play_card once tough is on a turf', () => {
    const state = makeState(weapon('w1'), [[tough('t1')]]);
    const actions = enumerateLegalActions(state, 'A');
    const playWeapon = actions.filter(
      (a) => a.kind === 'play_card' && a.cardId === 'w1',
    );
